...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Beryl database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Beryl datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Beryl datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Beryl 0.7 datadir."
    ;;
  3)
    echo "Detected Beryl pre-0.8 datadir."
    ;;
  4)
    echo "Detected Beryl 0.8 datadir."
    ;;
esac
