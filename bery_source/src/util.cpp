...
const char * const BITCOIN_CONF_FILENAME = "beryl.conf";
const char * const BITCOIN_PID_FILENAME = "beryld.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "beryl" is a composite category enabling all Beryl-related debug output
                if(ptrCategory->count(std::string("beryl"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Beryl
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Beryl
    // Mac: ~/Library/Application Support/Beryl
    // Unix: ~/.beryl
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Beryl";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Beryl";
#else
    // Unix
    return pathRet / ".beryl";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty beryl.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override beryl.conf
